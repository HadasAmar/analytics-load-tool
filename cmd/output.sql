...
SELECT campaign_id,
   ...
   media_source,
   event_name,
   SUM(inappevents_count) AS count,
   SUM(revenue) AS sum,
   SUM(first_inapps) AS new,
   SUM(organic_inappevents_count) AS organic_count,
   SUM(organic_revenue) AS organic_sum,
   SUM(organic_first_inapps) AS organic_new 
FROM ltv 
WHERE (batch <= '2022-09-18') 
    AND (((app_id = 'kuku') 
...
SELECT campaign_id,
   ...
   SUM(impressions_count) AS impressions,
   SUM(clicks_count) AS clicks,
   SUM(impression_installs_count) AS impression_installs,
   SUM(click_installs_count) AS click_installs,
   SUM(organic_installs_count) AS organic_installs,
   SUM(organic_impressions_count) AS organic_impressions,
   SUM(organic_clicks_count) AS organic_clicks,
   SUM(organic_impression_installs_count) AS organic_impression_installs,
   SUM(organic_click_installs_count) AS organic_click_installs 
FROM ltv 
WHERE (batch <= '2022-09-18') 
    AND ((app_id = 'com.kuku') 
    AND (unmasked_media_source = 'ms') 
